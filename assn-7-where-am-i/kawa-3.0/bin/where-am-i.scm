...
;;1
(define (single-intersection-points first others)
    (if (or (null? first) (null? others)) '()
        (append (intersect first (car others)) (single-intersection-points first (cdr others)))))

(define (intersection-points list)
    (if (or (null? list) (< (length list) 2)) '()
        (append (single-intersection-points (car list) (cdr list)) (intersection-points (cdr list)))))


;;2
(define (distance-product point list)
    (apply * (map (lambda (elem) (let ((distance (dist point elem))) 
        (if (= distance 0) 1
            distance))) list)))
...
(define (rate-points ls)
    (map (lambda (elem) (cons (distance-product elem (remove elem ls)) (list elem))) ls))

  
;;4
(define (point-partition pivot ls)
  (if (null? ls) '(() ())
      (let ((split-of-rest (point-partition pivot (cdr ls))))
	(if (< (car (car ls)) (car pivot))
...
(define (sort-points ls)
  (if (<= (length ls) 1) ls
      (let ((split (point-partition (car ls) (cdr ls))))
...
;;6
(define (avg-coordinates ls)
    (if (null? ls) (list 0 0)
    (list (/ (apply + (map car ls)) (length ls)) (/ (apply + (map cadr ls)) (length ls)))))

(define (average-point ls)
    (let ((avg (avg-coordinates ls)))
        (list (distance-product avg ls) avg)))
...
;;8
(define (where-am-i dist points)
    (sort-points (map best-estimate (all-guesses dist points))))
